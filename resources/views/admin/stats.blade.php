            ...
            <div class="card bg-dark border-secondary">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <div class="ms-2">
                            <select class="form-select form-select-sm chart-type-selector" data-chart="chartMostViewed">
                    ...
                    <div class="mt-2">{!! $mostViewed->appends(request()->query())->links() !!}</div>
                </div>
            </div>
        </div>
            ...
            <div class="card bg-dark border-secondary">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <div class="ms-2">
                            <select class="form-select form-select-sm chart-type-selector" data-chart="chartCritics">
                                <option value="bar" selected>Bar</option>
                                <option value="line">Line</option>
                                <option value="pie">Pie</option>
                            </select>
                    ...
                    <div class="mt-2">{!! $criticsChoice->appends(request()->query())->links() !!}</div>
                </div>
            </div>
        </div>
            ...
            <div class="card bg-dark border-secondary">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <div class="ms-2">
                            <select class="form-select form-select-sm chart-type-selector" data-chart="chartCritiqued">
                                <option value="bar" selected>Bar</option>
                                <option value="line">Line</option>
                                <option value="pie">Pie</option>
                            </select>
                    ...
                    <div class="mt-2">{!! $mostCritiqued->appends(request()->query())->links() !!}</div>
                </div>
            </div>
        </div>
            ...
            <div class="card bg-dark border-secondary">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        ...
                        <div class="ms-2">
                            <select class="form-select form-select-sm chart-type-selector" data-chart="chartEngaging">
                                <option value="bar" selected>Bar</option>
                                <option value="line">Line</option>
                                <option value="pie">Pie</option>
                            </select>
                    ...
                    <div class="mt-2">{!! $mostEngaging->appends(request()->query())->links() !!}</div>
                </div>
            </div>
        </div>
    </div>
...
function createChart(id, type, labels, data, labelText, color){
    // Normalize and trim
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if (chartData.mostViewed.labels.length) createChart('chartMostViewed','bar', chartData.mostViewed.labels, chartData.mostViewed.data, 'Views', 'rgba(54,162,235,0.7)');
    if (chartData.critics.labels.length) createChart('chartCritics','bar', chartData.critics.labels, chartData.critics.data, 'Avg Rating', 'rgba(255,205,86,0.7)');
    if (chartData.critiqued.labels.length) createChart('chartCritiqued','bar', chartData.critiqued.labels, chartData.critiqued.data, 'Reviews', 'rgba(153,102,255,0.7)');
    if (chartData.engaging.labels.length) createChart('chartEngaging','bar', chartData.engaging.labels, chartData.engaging.data, 'Engagement', 'rgba(75,192,192,0.7)');
    ...
    document.querySelectorAll('.chart-type-selector').forEach(sel => {
        sel.addEventListener('change', function(){
            ...
            // Map chartId to chartData key and params
            if (chartId === 'chartMostViewed') createChart(chartId, type, chartData.mostViewed.labels, chartData.mostViewed.data, 'Views', 'rgba(54,162,235,0.7)');
            if (chartId === 'chartCritics') createChart(chartId, type, chartData.critics.labels, chartData.critics.data, 'Avg Rating', 'rgba(255,205,86,0.7)');
            if (chartId === 'chartCritiqued') createChart(chartId, type, chartData.critiqued.labels, chartData.critiqued.data, 'Reviews', 'rgba(153,102,255,0.7)');
            if (chartId === 'chartEngaging') createChart(chartId, type, chartData.engaging.labels, chartData.engaging.data, 'Engagement', 'rgba(75,192,192,0.7)');
        });
    });
});
</script>
